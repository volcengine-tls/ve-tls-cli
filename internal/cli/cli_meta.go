...
func visibleCliGroups() []cliGroupSpec {
	return []cliGroupSpec{
		...
		{Name: "upgrade", Description: "显式检查或升级当前 CLI（不会后台联网）", Primary: true},
		{Name: "version", Description: "输出机器可读的版本、构建与 catalog 元信息", Primary: true},
		{Name: "login", Description: "Console Login Device Code 登录", Primary: true},
