...
func TestShortcutSuccessIsWrappedInEnvelope(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	if out["action"] != "project.list" {
		t.Fatalf("unexpected action: %v", out["action"])
	}
	if out["requestId"] != "req-project-list" {
		t.Fatalf("unexpected requestId: %v", out["requestId"])
	}
	if _, ok := out["data"].(map[string]any); !ok {
		t.Fatalf("expected wrapped data object: %v", out["data"])
	}
}

func TestShortcutServerErrorIsWrappedInEnvelope(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("x-tls-requestid", "req-project-create")
		w.WriteHeader(http.StatusConflict)
		_, _ = w.Write([]byte(`{"ErrorCode":"ProjectAlreadyExist","ErrorMessage":"Project already exists"}`))
	}))
	defer srv.Close()
	...
	var stdout, stderr bytes.Buffer
	code := Run([]string{"project", "create", "--project-name", "demo"}, &stdout, &stderr)
	if code != 2 {
		t.Fatalf("unexpected exit=%d stdout=%q stderr=%q", code, stdout.String(), stderr.String())
	}
	if strings.TrimSpace(stderr.String()) != "" {
