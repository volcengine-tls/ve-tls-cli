...
// TestResolveWorkloadSourceCredential is a coverage-strengthening table-driven
// test for resolveWorkloadSourceCredential. It verifies inline, pure cred-ref,
// partial merge precedence, missing source, and missing/broken cred-ref cases.
func TestResolveWorkloadSourceCredential(t *testing.T) {
	...
	t.Setenv("HOME", "/nonexistent-poison-home")
	...
	cases := []struct {
		...
		{
			name:    "inline_ak_sk_token",
			profile: config.Profile{AccessKeyID: "inline-AK", SecretAccessKey: "inline-SK", SecurityToken: "inline-TOK"},
			wantAK:  "inline-AK", wantSK: "inline-SK", wantTok: "inline-TOK",
		},
		{
			name:    "pure_cred_ref",
			profile: config.Profile{CredRef: "myref", SecurityToken: "profile-source-token"},
			creds:   map[string]config.Credential{"myref": {AccessKeyID: "ref-AK", SecretAccessKey: "ref-SK"}},
			wantAK:  "ref-AK", wantSK: "ref-SK", wantTok: "profile-source-token",
		},
		{
			name:    "partial_inline_ak_overrides_ref_ak",
			profile: config.Profile{AccessKeyID: "inline-AK", CredRef: "myref"},
			creds:   map[string]config.Credential{"myref": {AccessKeyID: "ref-AK", SecretAccessKey: "ref-SK"}},
			wantAK:  "inline-AK", wantSK: "ref-SK",
		},
		{
			name:    "partial_inline_sk_overrides_ref_sk",
			profile: config.Profile{SecretAccessKey: "inline-SK", CredRef: "myref"},
			creds:   map[string]config.Credential{"myref": {AccessKeyID: "ref-AK", SecretAccessKey: "ref-SK"}},
			wantAK:  "ref-AK", wantSK: "inline-SK",
		},
		{
			name:    "missing_source_fails_even_with_env_poison",
			profile: config.Profile{},
			wantErr: true,
		},
		{
			name:    "missing_cred_ref_fails",
			profile: config.Profile{CredRef: "nonexistent"},
			wantErr: true,
		},
		{
			name:    "broken_cred_ref_missing_ak_fails",
			profile: config.Profile{CredRef: "myref"},
			creds:   map[string]config.Credential{"myref": {SecretAccessKey: "ref-SK"}},
			wantErr: true,
		},
		{
			name:    "broken_cred_ref_missing_sk_fails",
			profile: config.Profile{CredRef: "myref"},
			creds:   map[string]config.Credential{"myref": {AccessKeyID: "ref-AK"}},
			wantErr: true,
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			cfg := config.Config{Creds: tc.creds}
			got, err := resolveWorkloadSourceCredential(tc.profile, cfg)
			if tc.wantErr {
				if err == nil {
			...
			if got.AccessKeyID != tc.wantAK {
				t.Fatalf("AK=%q, want %q", got.AccessKeyID, tc.wantAK)
			}
			if got.SecretAccessKey != tc.wantSK {
				t.Fatalf("SK=%q, want %q", got.SecretAccessKey, tc.wantSK)
			}
			if got.SessionToken != tc.wantTok {
				t.Fatalf("Token=%q, want %q", got.SessionToken, tc.wantTok)
			}
		})
	}
}
