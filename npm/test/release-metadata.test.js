...
test('npm publish guard keeps stable and release candidate dist-tags separate', () => {
  ...
  const stableLatest = spawnSync(process.execPath, [script], {
    env: { ...process.env, npm_package_version: '1.1.1', npm_config_tag: 'latest' },
    encoding: 'utf8',
  });
  const stableRC = spawnSync(process.execPath, [script], {
    env: { ...process.env, npm_package_version: '1.1.1', npm_config_tag: 'rc' },
    encoding: 'utf8',
  });
  const rcLatest = spawnSync(process.execPath, [script], {
    env: { ...process.env, npm_package_version: '1.1.2-rc.1', npm_config_tag: 'latest' },
    encoding: 'utf8',
  });
  const rc = spawnSync(process.execPath, [script], {
    env: { ...process.env, npm_package_version: '1.1.2-rc.1', npm_config_tag: 'rc' },
    encoding: 'utf8',
  });
