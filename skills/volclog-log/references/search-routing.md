...
| 写日志 / put logs / ingest logs | `volclog log ingest --describe` | 文本或 JSON 批量写入, 更适合先验证写入链路 |
| WebTracking / 前端埋点 | `volclog api log WebTracks --describe` | 更偏前端埋点写入 |

## Query Heuristics

- `Query` 不带分析段: 优先 `log search` 或 `log export`
- `Query` 中带 `|select`, `|with`, `|insert`, 聚合函数, 统计语义: 优先 `log export-analysis`
- 用户说“写日志 / put logs / ingest logs / web tracking”时，直接退出这个 routing，改走日志写入路径
