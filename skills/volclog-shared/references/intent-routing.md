...
| 写日志, put logs, ingest logs, web tracks, web tracking | 日志写入 | `log` | `volclog log ingest --describe` | 需要 raw PutLogs、WebTracking 或更特殊写入接口 |
| assistant, session answer, AI 助手, 智能问答 | 助手问答 | `assistant`（仅当前环境显式暴露时） | `volclog capabilities --view groups` | 只有当前环境明确暴露 assistant 能力或用户直接给出 assistant 命令时继续 |
...
| collector, 采集器, 采集配置, 采集规则, bind host group, 机器组绑定, parse path, parse time | `collector` | `collector` | `volclog collector list --describe` | shortcut 不覆盖后，再进入 `api collector <Action> --describe` |
| alarm, 告警, 通知组, notify group, webhook, 飞书, 钉钉 | `alarm` | `volclog-alarm` | domain skill 不覆盖后，再进入 `api alarm <Action> --describe` |
| host group, machine group, 机器组, ECS 机器组, 主机纳管, auto update | `host-group` | `host-group` | `volclog host-group list --describe` | shortcut 不覆盖后，再进入 `api host-group <Action> --describe` |
...
- `write logs to tls` -> 日志写入 -> `log ingest`
