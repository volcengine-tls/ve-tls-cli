...
region 必须显式提供，不要依赖 endpoint / 域名推导。
...
```bash
volclog configure set --profile default --ak "$VOLCENGINE_ACCESS_KEY_ID" --sk "$VOLCENGINE_ACCESS_KEY_SECRET" --region cn-beijing --endpoint https://tls-cn-beijing.volces.com
